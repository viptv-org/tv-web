...
export const emptyTitleMenuChoice: TitleMenuChoiceView = {
  key: "cancel",
  label: "",
  icon: "",
  focusedIcon: "",
...
export function titleMenuChoice(
  key: TitleMenuAction,
  label: string,
): TitleMenuChoiceView {
  return {
    ...
    label,
    icon: menuIcon(key),
    focusedIcon: menuIcon(key, true),
    visible: true,
  };
}

/** Same action order/copy as the React TV held-card menu. */
export function titleMenuChoices(
  item: MediaItem,
  inQueue: boolean,
  saved: boolean,
): TitleMenuChoiceView[] {
  ...
  if (item.previousEpisode)
    choices.push(titleMenuChoice("previous", "Resume previous episode"));
  ...
  if (inQueue)
    choices.push(titleMenuChoice("hide", "Remove from Continue Watching"));
  choices.push(
    titleMenuChoice(
      "favorite",
      saved ? "Remove from My List" : "Add to My List",
