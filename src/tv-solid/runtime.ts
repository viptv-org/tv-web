...
let remoteInputSuspended = false;
export function suspendRemoteInput(suspended: boolean) {
  remoteInputSuspended = suspended;
}
...
export function installRemoteInput() {
  ...
  const down = (event: KeyboardEvent) => {
    if (remoteInputSuspended) return;
  ...
  const up = (event: KeyboardEvent) => {
    if (remoteInputSuspended) { releases.clear(); return; }
