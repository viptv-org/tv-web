...
export const emptyDiscoverCard: DiscoverCardView = {
  id: "",
  title: "",
  subtitle: "",
  image: "",
...
export function requestForCatalog(
  catalog: Catalog,
  ...
  if (
    catalogFilters(catalog).some(
      (filter) => filter.required && !values[filter.name]?.trim(),
    )
  )
    return undefined;
  ...
  return {
    ...
    extras: Object.fromEntries(
      Object.entries(other).filter(([, value]) => value !== ""),
    ),
  };
}
...
export function discoverChips(
  ...
  for (const candidate of groups) {
    ...
    const width = { movie: 136, series: 127, anime: 129, other: 127 }[
      candidate
    ];
    chips.push({
      label,
  ...
  for (let index = 0; index < inGroup.length; index++) {
    ...
    chips.push({
      ...
      x,
      width,
      selected: sameCatalog(inGroup[index], selected),
      visible: true,
    });
    x += width + 8;
  }
...
export function catalogForGroup(
  catalogs: readonly Catalog[],
  group: DiscoverGroup,
): Catalog | undefined {
