...
const widthOf = (label: string) =>
  Math.max(92, Math.round(label.length * 13 + 57));
...
export function projectLiveGuide(
  ...
  for (let slot = 0; slot < visible.length; slot++) {
    ...
    for (let index = 0; index < cells.length; index++) {
      ...
      programViews.push({
        id: `${channel.id}:${index}:${cell.start}`,
        row,
        index,
        channel: { ...channel },
        cell,
        ...
        width,
        title: cell.missing
          ? "No guide data — press OK to watch live"
          : cell.title,
  ...
  if (channel) {
    ...
    hero = {
      ...
      next: next
        ? `Next at ${clockTime(next.start, zone, false)} · ${next.title}`
        : !program && guides[channel.id]
          ? "No guide information. You can still watch this channel."
          : "",
      monogram: channelMonogram(channel.name),
      progress: airing ? elapsed(cell.start, cell.end, now) : 0,
      minutesLeft: airing
        ? `${Math.max(1, Math.ceil((program.end - now) / 60))} min left`
        : "",
      airing,
    };
  }
