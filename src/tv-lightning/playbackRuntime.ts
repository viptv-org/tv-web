...
export interface LightningPlaybackRuntime {
  readonly player: Player;
  readonly controller: PlaybackSessionController<MediaItem, MediaSource>;
  start(item: MediaItem, source: MediaSource, position: number): ReturnType<PlaybackSessionController<MediaItem, MediaSource>["start"]>;
  stop(): Promise<void>;
  dispose(): Promise<void>;
}

/** One adapter/controller instance for the TV player, using the pinned video package. */
export function createLightningPlaybackRuntime(
  api: TvApi,
  platform: TvPlatform,
  video: HTMLVideoElement,
  onSnapshot: (snapshot: PlayerSnapshot) => void,
): LightningPlaybackRuntime {
  ...
  const enginePlatform: PlayerPlatform = platform === "webos" ? "html5" : platform;
