...
export const screens = {
  ...
  PhPin: { path: '/tv/settings', backend: { signOutPin: true, wrongPin: true }, note: 'reached through Sign out, so the title reads "…to sign out"', steps: async h => { await h.activate('signout'); await h.button('Sign out'); await h.wait('text-save'); await h.type('1234'); await h.activate('text-save'); await h.waitText('Incorrect PIN'); } },
  ...
  PhFilterText: { path: '/tv/discover', steps: async h => { await h.activate(h.page.getByRole('button', { name: 'Latest digital' })); await h.settle(); await h.activate(h.page.getByRole('button', { name: /^Search catalog/ })); } },
  ...
  PhLiveDetails: { path: '/tv/live', steps: h => h.hold('live-channel-1') },
  ...
  DeskSettings: { path: '/tv/settings', steps: h => h.activate('settings-appearance') },
  ...
  DeskPin: { path: '/tv/settings', backend: { signOutPin: true }, note: 'reached through Sign out, so the title reads "…to sign out"', steps: async h => { await h.activate('signout'); await h.button('Sign out'); await h.wait('text-save'); await h.type('123'); await h.activate('text-save'); } },
  DeskSignOut: { path: '/tv/settings', steps: h => h.activate('signout') },
  DeskPlayback: { path: '/tv/settings/playback', note: 'no Playback engine row outside the Tauri runtime' },
  DeskEngine: { notReachable: 'the Playback engine setting exists only on the Tauri platform (native player)' },
  ...
  TvSearch: { steps: async h => { await h.tvGo('Search'); await h.fill(h.page.getByRole('textbox', { name: 'Search titles' }), 'naruto'); await h.settle(); await h.focus('key-a'); } },
  ...
  TvPin: { backend: { signOutPin: true }, steps: async h => { await settingsRow(h, 'signout'); await h.button('Sign out'); await h.wait('text-save'); await h.type('1234'); } },
  TvPinError: { backend: { signOutPin: true, wrongPin: true }, note: 'reached through Sign out, so the title reads "…to sign out"', steps: async h => { await settingsRow(h, 'signout'); await h.button('Sign out'); await h.wait('text-save'); await h.type('1234'); await h.activate('text-save'); await h.waitText('Incorrect PIN'); } },
  ...
  TvFilterText: { steps: async h => { await h.tvGo('Discover'); await h.activate('discover-catalog'); await h.button('Latest digital', { exact: false }); await h.settle(); await h.activate('discover-filter-search'); } },
