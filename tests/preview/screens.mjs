...
export const screens = {
  ...
  PhAddonInstall: { path: '/tv/settings/addons', steps: async h => { await h.activate('addon-add'); await h.fill(entryInput(h), 'http://addon.example/manifest.json'); await h.activate('text-save'); await h.waitText('HTTPS manifest'); } },
  ...
  PhItemMenuLive: { path: '/tv/my-list', backend: { liveFavorite: true, favorites: true }, note: 'held on My List (a held channel on Live opens programme details), so the menu offers Remove from My List', steps: h => h.hold('result-0') },
  ...
  DeskAddonInstall: { path: '/tv/settings/addons', backend: { addonInstallHang: true }, steps: async h => { await h.activate('addon-add'); await h.fill(entryInput(h), 'https://lordstreams.example/manifest.json'); await h.activate('text-save'); await h.waitText('Saving'); } },
  ...
  DeskLiveDetails: { path: '/tv/live', note: 'Halftime Report (next up) stands in for Squawk on the Street: clicking a current programme plays it', steps: h => h.activate(h.page.getByRole('button', { name: /^CNBC: Halftime Report/ })) },
