...
| Player pause/seek/repeat/debounce/cancel/rollback/tracks/exit | `tests/e2e/media-decode.spec.ts` — real DOM decode advances, pauses, seeks, ends and disposes; `tests/player/*.test.ts` — adapter/controller failures and replacement boundaries | `player-remote.spec.ts` covers direct preview/repeat/debounce/Back cancellation, managed seek/subtitle-Off, progress on Exit and Sources return. Remaining: overlay timeout/Back order, all track-dialog focus transitions and managed rollback in app UI |
