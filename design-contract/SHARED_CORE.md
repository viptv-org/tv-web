...
No new pages, buttons, input timings or geometry are introduced. Splash, profile chooser, Home, dialogs and player retain the design system reference screens (viptv-design-system/) and specs/behavior/roku-ux-contract.md, including tap/hold/Back/focus restoration. The avatar focus outline remains concentric with its image. Valid partial catalogs remain visible when another row fails; a normalization error must not blank unrelated rows.
