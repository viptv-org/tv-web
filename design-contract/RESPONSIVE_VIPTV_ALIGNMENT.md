...
7. **Route changes must not move the frame or the header.** The application scroller keeps a permanent vertical scrollbar slot, so navigating between a tall route (Home) and a short one (Search, Live TV, My List) cannot change the content width and shift the frame sideways; the guide's own timeline scroller does the same for categories whose row count crosses the guide height. Overlay-scrollbar platforms (phones, tablets, current macOS) show nothing for the slot and lose no width. The header's profile control occupies a fixed slot (144px at 900px and above, 48px below), so a late-loading avatar, a changed profile name or a missing profile control cannot reflow the brand, navigation or Watch on TV actions.
